...
void init()
{
    ...
    if (isStereo)
    {
        ...
        gpuValues[0]  = -1.0f; gpuValues[1] = gpuValues[4*columns+3] = -0.38f; gpuValues[4*columns+2] = 1.0f;
    }
    else
    {
        gpuValues.resize(2*columns);
        gpuValues[0]  = -1.0f; gpuValues[1] = gpuValues[2*columns-1] = 0.0f; gpuValues[2*columns-2] = 1.0f;
    }
...
void update()
{
    ...
    if (isStereo)
    {
        ...
        for (size_t i = 0; i < columns; ++i)
        {
            ...
            sum = std::abs(sum);
            gpuValues[2*i + 3] = (sum / 32768.0f / samples_per_column) - 0.38f;
        }
        ...
        for (size_t i = 0; i < columns; ++i)
        {
            ...
            sum = std::abs(sum);
            gpuValues[4*columns + 1 - 2*i] = (sum / 32768.0f / samples_per_column) - 0.38f;
        }
    }
    else
    {
...
int main()
{
    ...
    while(!glfwWindowShouldClose(window))
    {
        ...
        update();
        glBindVertexArray(VAO);
        glDrawArrays(GL_LINE_STRIP, 0, 2*columns+2);
        glfwPollEvents();    
